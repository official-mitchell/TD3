...
| 2026-03-20 | **CORS normalization + shared helper:** backend/src/lib/cors.ts: getCorsOrigin() strips trailing slash and trims (browser Origin has no slash). getCorsConfig() used by main.ts and socket-service.ts. Socket.IO: pass origin as array per docs. Startup log: "CORS origin: <value> or NOT SET". |
| 2026-03-20 | **CORS: use CORS_ORIGIN when set regardless of NODE_ENV:** Render Docker may not set NODE_ENV. getCorsConfig() now uses CORS_ORIGIN whenever it's set; falls back to localhost only when unset. CORS_ORIGIN=* allows all origins (debug only). |
